...
class Midi
{
  public:
    ...
    Midi (typename TCallback::data * userdata, Uart & serial)
      : _userdata (userdata)
        , _serial (serial)
        , _running_status (0)
        , _expected (0)
  ...
  private:
    ...
    void handle_finished (uint8_t byte)
    {
      ...
      switch (msb)
      {
        case 0x90:
          ...
          if (_data[1] == 0)
          {
            TCallback::note_off (_userdata, lsb);
          }

          else
          {
            TCallback::note_on (_userdata, lsb, _data[0]);
          }
        ...
        case 0xe0:
            TCallback::pitch_bend (_userdata, msb & 0x0f, (_data[0] & 0x7f) | (_data[1] & 0x7f) << 7);
            break;
    ...
    typename TCallback::data * _userdata;  
    Uart &                     _serial;
